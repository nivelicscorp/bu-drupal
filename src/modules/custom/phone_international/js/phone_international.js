((Drupal) => {
  Drupal.behaviors.PhoneInternational = {
    attach: () => {
      once('phoneInternational', '.phone_international-number').forEach(
        (field) => {
          ...
          const options = {
            ...
            geoIpLookup: (callback) => {
              fetch(new Request('//ipinfo.io/json'))
                ...
                .then((resp) => {
                  const countryCode =
                    resp && resp.country ? resp.country : country;
          ...
          const drupalSelector =
            field.parentNode.nextElementSibling.getAttribute(
              'data-drupal-selector',
            );
          const value =
            field.parentNode.nextElementSibling.getAttribute('value');
          field.parentNode.nextElementSibling.remove();
          field.nextElementSibling.setAttribute(
            'data-drupal-selector',
            drupalSelector,
          );
