...
use Drupal\shs\Utility\TermLineage;
...
class OptionsShsWidget extends OptionsSelectWidget implements ContainerFactoryPluginInterface {
  ...
  public static function defaultSettings() {
    $settings_default = [
      ...
      'force_deepest' => FALSE,
      'save_lineage' => FALSE,
      'hide_add_another_item_button' => FALSE,
    ];
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $element['save_lineage'] = [
      ...
      '#default_value' => $this->getSetting('save_lineage'),
      '#description' => t('Save term lineage.'),
    ];
    $element['hide_add_another_item_button'] = [
      '#type' => 'checkbox',
      '#title' => t('Hide add another button'),
      '#default_value' => $this->getSetting('hide_add_another_item_button'),
    ];
  ...
  public function settingsSummary() {
    ...
    if ($this->getSetting('save_lineage')) {
      $summary[] = t('Save lineage');
    }
    else {
      $summary[] = t('Do not save lineage');
    }
    if ($this->getSetting('hide_add_another_item_button')) {
      $summary[] = t('Hide add another button');
    }
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $settings_additional = [
      ...
      'hideAddAnotherButton' => $this->getSetting('hide_add_another_item_button'),
    ];
    ...
    if ($default_value) {
      $parents = $this->widgetDefaults->getParentDefaults($default_value, $settings_additional['anyValue'], $this->fieldDefinition->getItemDefinition()
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    // Check save lineage setting.
    $save_lineage = $this->getSetting('save_lineage');
    if ($save_lineage) {
      // Term id list.
      $tids = [];
      // Vocabulary ID.
      ...
      if (!empty($values)) {
        foreach ($values as $delta => $value) {
          if (isset($value['target_id'])) {
            $arr_exploded_values = [];
            if (str_contains($value['target_id'], ',')) {
              $arr_exploded_values = explode(',', $value['target_id']);
              $value['target_id'] = end($arr_exploded_values);
            }
            elseif (str_contains($value['target_id'], ' ')) {
              $arr_exploded_values = explode(' ', $value['target_id']);
              $value['target_id'] = end($arr_exploded_values);
            }
            // Store target tid.
            if (!empty($arr_exploded_values)) {
              $tids += array_values($arr_exploded_values);
            }
            else {
              $tids[] = $value['target_id'];
            }
            if (empty($vid)) {
              // Load vid once, simpler to get the vocab ref this way.
      ...
      // Get term lineage from selected tid set.
      $handler = new TermLineage();
      $params = ['vid' => $vid];
      $lineages = $handler->getLineages($tids, $params);
      // Get tid list from lineage sets.
      $tid_list = $handler->getFlatLineage($lineages);
      // Save lineage set to values.
      $values = array_keys($tid_list);
    }
    ...
    if (str_contains($values[0], ',')) {
      $exploded_values = explode(',', $values[0]);
      $values = [];
    }
    elseif (str_contains($values[0], ' ')) {
      $exploded_values = explode(' ', $values[0]);
      $values = [];
    }
  ...
  public static function validateElement(array $element, FormStateInterface $form_state) {
    ...
    // If the terms lineage is stored the parents should be removed.
    if (!empty($value) && !empty($element['#shs']['settings']['save_lineage'])) {
      /** @var \Drupal\taxonomy\TermStorageInterface $term_storage */
      $term_storage = \Drupal::entityTypeManager()->getStorage('taxonomy_term');
      foreach ($value as $key => $item) {
        if (($children = $term_storage->loadChildren($item)) && array_intersect(array_keys($children), $value)) {
          unset($value[$key]);
        }
      }
